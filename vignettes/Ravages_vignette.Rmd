...
Ravages was developped to simulate genetic data and to perform rare variant association tests (burden tests and the variance-component test SKAT) on different types of phenotypes (Bocher et al., 2019, doi:10.1002/gepi.22210, Bocher et al., 2020, doi:10.1038/s41431-020-00792-8). This can be done at a genome-wide scale using the RAVA-FIRST approach which uses distribution of the CADD scores in the general population GnomAD to define genomic regions in the whole genome and filter genetic variants (Bocher et al. 2022, doi: 10.1371/journal.pgen.1009923). Ravages relies on the package Gaston developped by Herve Perdry and Claire Dandine-Roulland. Most functions are written in C++ thanks to the packages Rcpp, RcppParallel and RcppEigen.  
...
To cite the package Ravages, please refer to the corresponding application note: Bocher et al. 2023, Genetic Epidemiology: 10.1002/gepi.22529
...
From v1.2.0 onwards, all functions in Ravages, and especially the RAVA-FIRST strategy, can be applied on data in build 37 and 38 by changing the argument *build*.


## Example of analysis using LCT data
Below is an example of an association analysis and previous steps of data filtering using the plink-format files `LCT.EUR.*` available with the package Ravages. This dataset containts data from the 1000Genomes project (anonymized by shuffling the genomic positions) in the Lactase gene in individuals of European ancestry. In this example, we look for an association between rare variants and the European subpopulations of 1000Genomes. The population of each
individual is available in the sample information file `LCT.EUR.*.fam`. A classical analysis by gene is performed, and an analysis using the strategy "RAVA-FIRST".  
...
The files **genes.b37** and **genes.b38** available in Ravages which contain gene positions from ENSEMBL versions GRCH37 and GRCH38 can be used as *regions*. The corresponding build should be specified in subsequent functions by the argument *build*.   
...
To attribute adjusted CADD scores to variants, the function **adjustedCADD.annotation()** can be used. It will call the function **adjustedCADD.annotation.SNVs()** and **adjustedCADD.annotation.indels()** to annotate SNVs and indels respectively. As CADD scores are available for every SNVs in the genome, SNVs in the bed matrix will be annotated using a provided file with *adjusted CADD scores* to *SNVs.scores* (with columns 'chr', 'pos', 'A1', 'A2', 'adjCADD'), or by downloading the adjusted scores from https://lysine.univ-brest.fr/RAVA-FIRST/ in a repository indicated to *path.data*. Oppositely, pre-computed PHRED CADD scores are not available for every indel in the genome and they should be annotated online in https://cadd.gs.washington.edu/. To computed adjusted CADD scores for indels, we used a set of 48M indels available at https://cadd.gs.washington.edu/download for build 27, and all the indels observed in GnomAD v1.3.2 for build 38. Indels in the bed matrix will then be annotated using the file AdjustedCADD_v1.4_202204_indels.tsv.gz/AdjustedCADD_v1.6_202602_indels.tsv.gz downloaded from https://lysine.univ-brest.fr/RAVA-FIRST/ and if new indels not present in the set of predefined indels are to be annotated, they will be given the same adjusted score as the indel with the nearest PHRED CADD score. Therefore, if indels are present in the bed matrix, a file with the *CADD PHRED v1.4* (build 37) or *CADD PHRED v1.6* (build 38) score should be given to *indels.scores* in **adjustedCADD.annotation()** (with columns 'chr', 'pos', 'A1', 'A2', 'CADD').  

To assign variants to CADD regions and to the functional categories, the function **set.CADDregions()** can be used. It will add *genomic.region* to the bed matrix with the corresponding CADD regions present in the file "CADDRegions.202204.hg19.bed.gz" or "CADDRegions.202602.b38.bed.gz" (depending on the build) and *SubRegion* with the functional area present in the file "FunctionalAreas.hg19.bed.gz" or "FunctionalAreas.b38.SCREENv3.bed.gz", both directly downloaded from https://lysine.univ-brest.fr/RAVA-FIRST/ in the repository defined in *path.data*. In addition, *adjCADD.Median* will be added to *x@snps* which correspond to the median adjusted CADD score of variants (SNVs + indels) observed at least two times in GnomAD and can be used for variant filtering as explained later.
