filter.adjustedCADD <- function(x, SNVs.scores = NULL, indels.scores = NULL, ref.level = NULL, filter = c("whole", "controls", "any"), maf.threshold = 0.01, min.nb.snps = 2, min.cumulative.maf = NULL, group = NULL, cores = 10, path.data, verbose = T, build = c("b37", "b38")){
  ...
  if(!("adjCADD" %in% colnames(x@snps))){  
    ...
    x <- adjustedCADD.annotation(x, SNVs.scores = SNVs.scores, indels.scores = indels.scores, cores = cores, verbose = verbose, path.data = path.data, build = build)
  }
  else{
